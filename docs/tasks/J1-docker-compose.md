...
- [ ] `backend/app/main.py`가 있고 `uvicorn app.main:app`으로 뜬다 — **팀장이 만드는 앱 뼈대.** 없으면 기다린다
...
이 PR을 올렸으면 **[W1-3](../weekly/W1-3.md)의 "목업이 끝난 사람은 즉시 다음으로" 표에서 자기 다음 작업을 확인한다.** 표에 다음이 없으면 [J7 파트A](J7-더미데이터-생성기.md) 배치를 가져가고, 팀 채널에 "J7 배치 N 가져감"을 남긴다. **목록에 없는 일을 스스로 만들지는 않는다.**
