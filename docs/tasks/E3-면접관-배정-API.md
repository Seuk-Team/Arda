...
**A3(면접관은 본인 배정 지원자만 조회)는 필수 기능인데, 배정 관계가 없으면 강제할 방법이 없다.** [01-erd.md](../01-erd.md)가 `interviewer_assignments` 를 둔 이유가 이것이다.
...
- **`role="interviewer"` 인 사용자만 배정한다.** 아무나 배정되면 이 테이블의 의미가 없다.
...
- [ ] `role != "interviewer"` 인 사용자를 배정하면 **422**
...
- [docs/01-erd.md](../01-erd.md) — `interviewer_assignments` 표와 A3 설명
- [docs/02-api.md](../02-api.md) — 권한 규칙 (interviewer 는 본인 배정 지원서만)
- [docs/planning/00_summary_ko.md](../planning/00_summary_ko.md) — A3 · E3 기능 정의
...
docs/01-erd.md 의 interviewer_assignments 표와 그 위 설명, docs/02-api.md 상단의 권한 규칙을 읽어라.
