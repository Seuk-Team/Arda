...
- [docs/01-erd.md](../01-erd.md) — `applications` · `stage_history` · `evaluations` · `application_notes` · `files`
- [docs/02-api.md](../02-api.md) — "지원자 관리 (D·H)" 표
...
```
0단계.
backend/app/models.py 전체, docs/01-erd.md 의 applications 와 그 자식 테이블들,
docs/02-api.md 의 "지원자 관리 (D·H)" 표, frontend/mockups/mockup.html 의 상세 패널을 읽어라.
