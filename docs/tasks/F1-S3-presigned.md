...
**이 프로젝트에서 AWS 를 실제로 쓰는 자리다.** 발표에서 "파일 업로드가 왜 API 서버를 거치면 안 되는가"를 말할 근거가 여기서 나온다 ([04-team.md](../04-team.md) C 역할).
...
- **`s3_key` 컬럼명은 아직 미결이다** ([01-erd.md](../01-erd.md) 미결 항목). 지금은 그대로 쓰고, 바뀌면 팀장이 일괄 변경한다.
...
- [docs/01-erd.md](../01-erd.md) — `files` 표와 `s3_key` 미결 항목
- [docs/02-api.md](../02-api.md) — "지원 — 공개 (C)" 의 presign 행
...
```
0단계.
backend/app/models.py 의 File 클래스와 FILE_KINDS, docs/01-erd.md 의 files 표,
docs/02-api.md 의 presign 관련 행, backend/.env.example 을 읽어라.
