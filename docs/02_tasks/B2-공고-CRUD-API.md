...
- [ ] [docs/00_overview/01-erd.md](../00_overview/01-erd.md)가 **확정 상태**다 (문서 상단이 `초안`이 아니라 `확정 vX.X · 날짜`)
...
- [ ] [docs/00_overview/02-api.md](../00_overview/02-api.md)의 공고 표와 경로·메서드가 정확히 일치한다 (문서를 고치지 말고 코드를 맞춘다)
...
```
0단계.
backend/app/models.py, backend/app/db.py, docs/00_overview/01-erd.md 의 job_postings 표,
docs/00_overview/02-api.md 의 "채용 공고 (B)" 표를 읽어라.
