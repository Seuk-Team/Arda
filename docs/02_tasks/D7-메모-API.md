...
평가(`evaluations`)는 점수 1~5 가 필수인 행이다. "전화 안 받음", "다음 주 재연락" 같은 **점수 없는 기록이 섞이면 평가 목록과 평균이 오염된다.** 그래서 [01-erd.md](../00_overview/01-erd.md)가 `application_notes` 를 따로 뒀다.

**한 문서를 여럿이 고치는 구조가 아니라 각자 행을 추가하는 구조**다 ([ADR-0005](../03_decision/0005-실시간-공동편집-제외.md)). 그래서 동시 편집 충돌 처리가 필요 없다 — 다만 **본인 메모 수정 시에는** 덮어쓰기 감지가 필요하다.
...
- **작성자 검사는 코드에서 한다** ([01-erd.md](../00_overview/01-erd.md) 명시). 인증이 아직 없으므로 지금은 자리만 만들고 `TODO(A1)` 주석을 남긴다.
...
- [ ] **덮어쓰기 감지**: `PATCH` 본문에 `updated_at` 을 함께 받아 DB 값과 다르면 **409** ([ADR-0005](../03_decision/0005-실시간-공동편집-제외.md))
- [ ] 없는 지원자·메모 → **404**
...
- [docs/00_overview/01-erd.md](../00_overview/01-erd.md) — `application_notes` 표와 그 위 설명
- [docs/00_overview/02-api.md](../00_overview/02-api.md) — "메모" 표
- [ADR-0005](../03_decision/0005-실시간-공동편집-제외.md) — 왜 공동 편집을 안 하는가
...
```
0단계.
docs/00_overview/01-erd.md 의 application_notes 표와 바로 위 설명, docs/00_overview/02-api.md 의 메모 표,
docs/03_decision/0005-실시간-공동편집-제외.md 를 읽어라.
