...
점수는 **1~5 체크 제약**이 DB 에 이미 걸려 있다([01-erd.md](../00_overview/01-erd.md)). API 도 같은 범위로 막아 **DB 예외가 500 으로 새어 나가지 않게** 한다 — 422 로 돌려줘야 화면이 사용자에게 설명할 수 있다.
...
- **본인 평가만 수정(E5)은 코드에서 검사한다** ([01-erd.md](../00_overview/01-erd.md) 명시). 인증이 없으므로 자리만 만들고 `TODO(A1)`.
...
- [docs/00_overview/01-erd.md](../00_overview/01-erd.md) — `evaluations` 표와 비고(E4·E5)
- [docs/00_overview/02-api.md](../00_overview/02-api.md) — "평가 (E)" 표
...
```
0단계.
backend/app/models.py 의 Evaluation, docs/00_overview/01-erd.md 의 evaluations 표와 비고,
docs/00_overview/02-api.md 의 평가 표, frontend/mockups/mockup-evaluations.html,
