...
> **2026-09-07 주석 (팀장)**: 아래 본문은 **현재 구현**의 근거다 — 오디오만 · 답변 단위 녹음 → S3 · 재시도 허용 · 텍스트 폴백(PR #40 · #50). 이것은 잠금이 아니다. **영상+오디오, 실시간 스트리밍, 재시도 정책, 폴백 정책**은 §7 확장 후보에 있고, 오너가 개정 ADR 로 추가할 수 있다([03-conventions "결정 문서 개정"](../00_overview/03-conventions.md)). 인프라·비용 조건은 §7 표에 적어 뒀다.
...
**영상은 저장하지 않는다 — 현재 결정(v1.6).** 대리 응시 확인·표정 판별이 필요하면 실시간 표시만 하고 파일을 남기지 않는다 — 저장하는 순간 민감정보 보관 의무가 붙는데 얻는 것에 비해 부담이 크다([ADR-0026](../03_decision/0026-AI-면접-음성분석-제외.md)). **개정하려면** 보관 기간·동의 문구·별도 테이블·S3 수명주기가 같이 와야 한다 — §7.
...
## 7. 확장 후보 — 구조적으로 추가할 수 있는 것 (2026-09-07)

현재 구현(오디오 · 답변 단위 · 재시도 허용 · 텍스트 폴백)과 **반대 방향의 설계가 하나 있었다**(영상+오디오 · 실시간 스트리밍 · 재시도 없음 · 못 하면 면접 불가). 어느 쪽이 "맞다" 가 아니라, 현재 구현을 **바탕**으로 두고 아래를 **얹을 수 있다**는 목록이다. 각 줄은 오너(백엔드 woojeongalex · 에이전트 cloverky · 프론트 minahdev)가 개정 ADR 로 채택하면 들어간다. 팀장이 보는 것은 오른쪽 두 열(인프라·비용)뿐이다.

| 후보 | 지금 | 얹으면 얻는 것 | 같이 와야 하는 것 | 인프라·비용 (2026-09-07 기준) |
|---|---|---|---|---|
| **영상+오디오** | 오디오만 저장. 영상은 저장 안 함 | 표정·음성 진위 판별([ADR-0029](../03_decision/0029-표정-음성-진위-판별-도입.md)) 입력, 대리 응시 확인 | 저장하지 않고 **실시간으로 `/ai/analyze` 에 흘려보내는** 경로가 먼저(저장 의무 없음). 저장까지 가면 보관 기간·동의 문구·별도 테이블·S3 수명주기 | 거짓말 탐지 서비스는 이미 운영 EC2(t3.medium)에 떠 있다. 분석 1건 574MiB, 워커 1개 직렬. 동시 면접이 늘면 GPU/별도 서버 |
| **실시간 스트리밍** | 답변 단위 녹음 → S3 → 전사 | 끊김 없는 대화감, 실시간 진행 보조 | WebSocket/WebRTC 경로 · 부분 전사(스트리밍 STT) · 끊겼을 때 복구 규칙. 답변 단위 경로는 **폴백으로 유지** | Caddy 뒤에 WebSocket 은 됨. 스트리밍 STT 는 OpenAI Realtime 또는 GPU 서버 faster-whisper — 둘 다 비용·지연 실측 필요. 발표(09/25) 전 착수는 권하지 않음 |
| **재시도 정책** | 답변마다 다시 하기 허용(`reset()`) | "첫 답" 신호가 필요하면 회수 제한 | 재시도 **횟수 기록**(`interview_turns` 컬럼) 이 먼저 — 막기 전에 재고 결정. 네트워크 끊김과 "다시 하기" 를 구분 | 없음 (스키마 한 줄) |
| **폴백 정책** | 텍스트로 답할 수 있음 | 전원 음성/영상으로 통일 | 마이크·카메라 없는 환경, 청각·언어 장애 지원자에 대한 대안 경로가 **반드시** 같이 와야 함(ADR-0029 ① 공정성). 텍스트 폴백을 없애는 안은 그 대안 없이는 성립하지 않음 | 없음 |

채택 순서 제안(팀장): **영상 실시간(저장 없음) → 재시도 횟수 기록 → 스트리밍**. 폴백은 대안 경로가 나올 때까지 유지.
