...
- **1차 완성: 09/30(수).** 주차 기준: W1 08/24~28 · W2 08/31~09/04 · W3 09/07~11 · W4 09/14~18 · W5 09/21~25 · 09/28~30 통합 버퍼.
- **주 단위로 스스로 계획하고 진행한다.** 각 오너의 주간 계획이 기본값이고, 도메인 간에 걸리는 변경만 [06-weekly.md](../00_overview/06-weekly.md)에서 맞춘다.
...
| **W2** (~09/04) | **M2 실배포 1차** | IAM 최소 권한 정리 · SQS 큐 생성 · SES 샌드박스 해제 확인 · EC2에 api+워커 수동 배포 · Vercel 연결 · `.env.example` 체계 확정 | 외부 URL에서 `/health` 200. 프론트 프리뷰가 실 API를 바라봄. 팀원 로컬에서 `.env.example`만 보고 환경 구성 가능 |
...
| # | 작업 | 비고 |
|---|---|---|
| 1 | AWS 계정 · S3 · SES 신청 | **오늘.** 늦으면 백엔드 9·13번(F·G)이 통째로 선다 |
| 2 | J0 리뷰·머지 (woojeongalex 작성 중, [PR #11](https://github.com/Team-Seuk/Arda/pull/11)) | 백엔드 전체의 선행 |
| 3 | E1·A2·QA 등 진행 중 PR 리뷰·머지 → **ERD 확정** | 확정 즉시 팀 채널 공지 |
| 4 | 스키마 합의 2건 결정 + 02-api 전역 검색 경로 추가 | 백엔드 6·19·20번 선행 |
| 5 | 도메인 오너제 전환 PR (로드맵 5종 + 문서 갱신 — 이 문서 포함) | 팀원 검수 후 머지 |
| 6 | GitHub 설정: 브랜치 보호(main 직접 push 금지 유지, 필수 승인 0으로 변경) + CODEOWNERS 갱신 | 리뷰 위임의 물리적 전제 |
| 7 | J1 Docker compose | [지시서](../02_tasks/J1-docker-compose.md) |
| 8 | mockup.html 색 리터럴 정리 (토큰 없는 7종 네이밍 포함) | 금요일. 프론트 M2 공통 CSS 파일화의 선행 |
| 9 | M2~M4 항목 순서대로 | |
...
- **SES 샌드박스 해제 지연** → 신청을 오늘 걸고, 해제 전엔 검증된 수신자(팀원 메일)로만 발송 테스트.
