...
| 9 | ~~M2 실배포 1차~~ ✅ (08/27 — [07-deploy](../00_overview/07-deploy.md) 신설) | 잔여: SES 해제 확인 · **초기 버전 게이트 판정(09/04 금)** |
...
- **SES 샌드박스 해제 지연** → 신청 완료·승인 대기 중(08/27 기준). 해제 전엔 검증된 수신자(팀원 메일)로만 발송 테스트.
