...
| GET | /applicant/me | 내 지원 현황 | **지원자 토큰 전용.** 조회 인자를 받지 않는다 — 토큰의 이메일로만 찾는다. 지원마다 `interviews[]`(들어갈 수 있는 면접의 `token`·`status`·`expires_at`)가 붙는다 |
...
- **면접 토큰을 `/applicant/me` 가 같이 내린다** (2026-09-08). 본인 토큰으로 조회한 자기 면접이라 새로 여는 비밀이 아니고, 이게 없으면 **로그인해 놓고도 메일함을 뒤져야 면접에 들어간다.** `pending`·`in_progress` 만 싣는다 — 끝났거나 만료된 면접은 들어가 봐야 막히므로 화면에 내지 않는다. 들어가는 곳은 지원자 쪽 `/interview-live/{token}`(실시간) 또는 `/interview/{token}`(AI 면접)이다
