...
| POST | /public/interview/{token}/finish | 면접 종료 | **공개**. **다 답하지 않아도 끝낼 수 있다.** 두 번 눌러도 200. **ADR-0034**: 끝나면 백그라운드로 아르가 채점해 `interview_sessions.ai_score` 를 채운다(담당자 상세 `GET /interview-sessions/{id}` 의 `ai_score`·`ai_score_detail`·`scored_at`) |
