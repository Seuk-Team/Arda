...
def class_weights(ds, device, mode="inverse"):
    ...
    **전체 정확도는 오히려 조금 내려갈 수 있다.** 우리가 보고 싶은 것은 긴장·불안
    ...
    ## 3차에서 드러난 것 — 너무 세게 주면 반대로 망가진다

    역겨움은 191장뿐이라 가중치가 **21.11**, 무표정(10,309장)은 **0.39** 였다.
    54배다. "역겨움을 틀리면 54배 손해" 라고 가르치면 모델은 **애매할 때마다
    역겨움이라 답하는 쪽**이 이득이다. 실제로 191장 배우고 780번 답했고, 그중
    527번이 무표정이었다(정밀도 5.9%).

    `mode="sqrt"` 는 그 보정을 눕힌다 — 21.11 → 4.59, 0.39 → 0.62 로 **54배가
    7배**가 된다. 적은 표정을 여전히 챙기되 과다 예측까지 가지는 않는다.
    """
...
def evaluate(model, loader, device, amp=False):
    """(전체 정확도, 표정별 재현율, 표정별 정밀도). **한 번만 돈다.**

    평가도 학습과 같은 정밀도로 한다. fp32 로 돌리면 학습이 잡아 둔 메모리가
    남아 있는 상태에서 8GB 를 넘긴다 — 첫 시도가 그렇게 죽었다.

    **정밀도를 같이 센다** (3차 이후). 재현율만 보면 과다 예측이 안 보인다 —
    역겨움 재현율 80.7% 뒤에 정밀도 5.9% 가 숨어 있었다.
    """
    model.eval()
    hit = torch.zeros(len(LABELS))
    total = torch.zeros(len(LABELS))
    said = torch.zeros(len(LABELS))      # 그 표정이라고 답한 횟수
    for images, labels in loader:
        ...
        for t, p in zip(labels.tolist(), pred.tolist()):
            ...
            hit[t] += t == p
    return (
        (hit.sum() / total.sum()).item(),
        (hit / total.clamp(min=1)).tolist(),
        (hit / said.clamp(min=1)).tolist(),
    )
...
def main():
    ...
    # inverse = 장수에 반비례(1~3차). sqrt = 그 제곱근 — 3차에서 역겨움 가중치가
    # 21배까지 올라가 과다 예측이 났다(정밀도 5.9%).
    p.add_argument("--weights", default="inverse", choices=["inverse", "sqrt"])
    # recall = 표정 평균 재현율(1~3차). f1 = 정밀도까지 같이 본다.
    p.add_argument("--select", default="recall", choices=["recall", "f1"])
    # 활성값을 들고 있지 않고 역전파 때 다시 계산한다 — 메모리를 계산으로 바꾼다.
    # 약 30% 느려지는 대신 활성값 자리가 크게 준다. 8GB 카드에서 **다른 프로그램이
    # VRAM 을 쓰고 있을 때**(크롬이 5GB 를 물고 있는 일이 흔하다) 이게 없으면
    # 첫 스텝부터 OOM 이다.
    ...
    if args.checkpointing:
        ...
        print("그래디언트 체크포인팅 켬 — 느린 대신 메모리를 아낀다")
    weights = None if args.no_weights else class_weights(train_ds, device, args.weights)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        acc, recalls, precs = evaluate(model, test_dl, device, amp=amp)
        ...
        f1s = [
            0.0 if r + pr == 0 else 2 * r * pr / (r + pr)
            for r, pr in zip(recalls, precs)
        ]
        macro_f1 = sum(f1s) / len(f1s)
        print("    표정별(재현/정밀): " + " · ".join(
            f"{KOREAN[n]} {r*100:.0f}/{pr*100:.0f}"
            for n, r, pr in zip(LABELS, recalls, precs)))
        print(f"    표정 평균  재현 {macro*100:.2f}%  ·  F1 {macro_f1*100:.2f}%")
        history.append({"epoch": epoch, "loss": loss_sum / seen, "accuracy": acc,
                        "macro_recall": macro, "macro_f1": macro_f1,
                        "per_class": dict(zip(LABELS, recalls)),
                        "per_class_precision": dict(zip(LABELS, precs))})

        # **가장 좋은 것만 남긴다.** 덮어쓰므로 폴더가 커지지 않는다.
        # 기준은 전체 정확도가 아니라 **표정 평균**이다 — 전체로 고르면 흔한
        # 표정(웃음 7,215장)만 잘 맞히는 모델이 뽑힌다.
        # **`--select f1` 을 쓰는 이유** (4차): 재현율만 보면 과다 예측이 이득이다.
        # "역겨움" 을 마구 답할수록 역겨움 재현율은 오르고, 정밀도가 5.9% 로
        # 내려가는 것은 이 값에 안 잡힌다. F1 은 둘을 같이 본다.
        score = macro_f1 if args.select == "f1" else macro
        if score > best:
            best = score
    ...
    (out / "history.json").write_text(
        json.dumps({"설정": vars(args), "best_score": best, "기준": args.select,
