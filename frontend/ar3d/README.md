...
감은 눈 덮개 다듬기(이미 적용돼 있다. 다시 돌려도 안전):

```bash
blender -b --python fix_eye_lid.py
```

감은 눈(confirm·fail)은 얼굴에 `ExLid` 덮개를 씌워 **텍스처에 그려진 눈**을 가리는 방식이라,
덮개가 원반처럼 도드라져 보였다. 색(얼굴 = 텍스처 × baseColorFactor 라 실제 `(0.456,0.543,0.277)`)·
roughness(0.5)·법선을 얼굴에 맞추고, 돔의 뒤쪽 절반을 지운 뒤 얼굴 표면 앞 1.0 에 밀착시켰다.
재질은 전부 `OPAQUE` 로 바꿨다(알파가 다 1.0 인데 `HASHED` 로 들어와 있어 겹친 면 판정이 흔들렸다).

**아직 옅은 원이 남아 있다.** 덮개는 그려진 눈을 가리려면 얼굴보다 최소 1.0 은 떠 있어야 하는데
(0.25 까지 붙이면 눈이 얼룩덜룩 비친다 — 재질을 전부 불투명으로 해도 같으니 알파가 아니라 기하 문제),
그 단차와 "단색 덮개 vs 텍스처 얼굴"의 차이가 완전히는 안 사라진다.
**뿌리부터 없애려면** 텍스처에서 눈을 지우고 뜬 눈도 `ExOrb` 같은 입체 눈으로 돌려서
덮개 자체를 없애야 한다 — 텍스처 수정 + 전 모션의 눈 표시 상태 변경이 필요한 큰 작업이다.
