...
로컬 sLLM 아르 채팅을 실 백엔드에 실호출해 성공률·응답 시간·라우팅 정확도를 기록한다. 라우터·프롬프트·모델 변경 후 이 하네스를 다시 돌려 개선/회귀를 숫자로 본다.
...
⚠️ **리셋은 `backend/.env` 의 DATABASE_URL 을 직접 읽는다.** `app.db` 는 env 가 없으면 `localhost:5432/postgres` 기본값으로 떨어지는데 이 PC 에는 네이티브 PostgreSQL 이 5432 에 살아 있어, 그대로 두면 엉뚱한 옛 DB 를 조용히 바꾼다 (실제로 겪었다). 서버가 쓰는 DB 는 5433(pgvector 컨테이너)이다.
...
1. `arda-pgvector` 컨테이너 기동: `docker start arda-pgvector`
2. 백엔드 실행 (`.env` 에 로컬 sLLM 세팅 or 클라우드 세팅 중 검증하고 싶은 쪽으로)
...
- `REGRESSION_BASE_URL=http://localhost:8000` — 기본값
- `REGRESSION_TAG=baseline` — 결과 파일명 태그 (기본 `run`)
  - 예: `baseline`, `router-v1`, `qwen3-4b-instruct` 등
...
- **router_hit** = 규칙 라우터가 잡은 시나리오 (2026-09-02 기준 `false` — Phase 1 확인/의도 라우터 도입 후 `true` 등장 예정)
- **tool_calls** = 도구 이름 리스트. 같은 도구 여러 번이면 폭주 가능성 (Guard 가 이제 잡음)

## 기준선 (2026-09-02, PR #161 Guard 도입 후)
...
Phase 1 라우터 도입 후 목표: **평균 <5초** (뻔한 시나리오 6~7개가 라우터로 0~2초).
