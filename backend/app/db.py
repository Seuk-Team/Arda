...
**2026-09-01 부터 마이그레이션은 alembic 이 맡는다** (`backend/alembic/`).
그 전까지의 방침("바뀌면 DB 를 지우고 다시 만든다")은 실지원자 데이터가 들어온
뒤로는 성립하지 않았다 — 같은 날 로컬 테스트 101건과 운영 AI 요약 전건이
`create_all` 이 기존 테이블을 안 고친다는 이유로 깨졌다.

역할 분담은 이렇다.

- `create_all` — **새 DB 를 세울 때만.** 여기와 main.py lifespan·conftest·create_admin
- `alembic upgrade head` — **바뀐 것을 따라갈 때.** 스키마를 고치면 리비전을 남긴다
"""
