...
class CompletionResult:
    ...
    output_tokens: int = 0
    cost_usd: float = 0.0
    # 한도에서 잘렸는지. 잘린 응답은 JSON 이 깨져 파싱에 실패하는데, 그것을 그냥
    # "파싱 실패"로만 보면 원인이 안 보인다 (2026-09-01 에 실제로 물렸다 — 96204f9).
    # Anthropic 은 "max_tokens", Ollama 는 done_reason "length" 를 그 값으로 준다.
