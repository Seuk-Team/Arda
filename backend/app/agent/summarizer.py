...
def _call_llm(backend, prompt_text: str, step_name: str) -> tuple[str, int, int, float]:
    """LLM 1회 호출. (응답 텍스트, input_tokens, output_tokens, cost_usd) 반환.
    ...
    return result.text.strip(), result.input_tokens, result.output_tokens, result.cost_usd
...
def generate_summary(db: Session, application_id: int) -> str | None:
    ...
    try:
        ...
        raw1, in1, out1, cost1 = _call_llm(backend, step1_text, "chain_summarize")
        total_input += in1
        total_output += out1
        total_cost += cost1
    except Exception:
    ...
    try:
        ...
        raw2, in2, out2, cost2 = _call_llm(backend, step2_text, "chain_evaluate")
        total_input += in2
        total_output += out2
        total_cost += cost2
    except Exception:
    ...
    try:
        ...
        raw3, in3, out3, cost3 = _call_llm(backend, step3_text, "chain_recommend")
        total_input += in3
        total_output += out3
        total_cost += cost3
    except Exception:
